          ...
          <form method="POST" action="../nedmin/admin_islem.php"   class="form-horizontal form-label-left">
            ...
            <div class="form-group">
              ...
              <div class="col-md-6 col-sm-6 col-xs-12">
                <select class="select2_single form-control" tabindex="-1" name="theme">
                  <option></option>
                  <?php 
                 ...
                 /* $ayarsor=$db->prepare("SELECT * FROM ayar where ayar_id=:id");
                  ...
                  $themesor=$db->prepare("SELECT * FROM theme ");
                  $themesor->execute();
                  while ( $themecek=$themesor->fetch(PDO::FETCH_ASSOC)) { ?>

                   <option <?php if($ayarcek['theme']==$themecek['theme_css_ad']){echo "selected";} ?>   value="<?php echo $themecek['theme_css_ad'] ?>"><?php echo $themecek['theme_css_ad'] ?></option>
                 <?php }?>
                </select>
              </div>
            </div>
            ...
            <div class="form-group">
              <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                <button type="submit" name="tema" class="btn btn-primary  col-md-12 col-xs-12">Y E N İ L Ə</button>
              </div>
            </div>
          </form>
        </div>
